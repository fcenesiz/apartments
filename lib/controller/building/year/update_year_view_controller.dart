...
import '../add_building_view_controller.dart';

class UpdateYearViewController extends GetxController {
  ...
  void setValuesByArguments() {
    ...
    if (building.years.isNotEmpty) {
      int idx = building.years.length - 1;
      number.value = building.years[idx].number;
      offsetMonth.value = months[building.years[idx].offsetMonth].toString();
      quantityController.text = building.years[idx].feeQuantity.toString();
      _year = building.years[idx];
    } else {
      _year = Year();
      number.value = DateTime.now().year;
    }
  }
